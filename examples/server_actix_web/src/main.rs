use actix_web::{App, HttpRequest, HttpServer, ResponseError, web::{Data, Json}};
...
async fn hello(request: HttpRequest, _data: Data<()>, body: HelloRequest) -> Result<Json<HelloResponse>, ServerError> {
    ...
    Ok(Json(HelloResponse {
        hellos: body.names.iter().map(|name| format!("Hello, {}!", name)).collect()
    }))
}
